---
title: "Applied Epi - Case Study Template"
tutorial:
  id: "template"
output: 
  learnr::tutorial:
...
description: >
  One sentence about the session and module
---

<!-- This is a Template that can be used to create an R case study in markdown -->
...
> Version XXX, DATE OF LAST UPDATE


## Disclaimer 

<!-- HERE YOU CAN WRITE A DISCLAIMER ABOUT THE DATA USED, COPYRIGHT, LICENCE -->
...
<!-- HERE LIST THE DIFFERENT REVISIONS MADE TO THE FIRST VERSION OF THE CASE STUDY

FOR EXAMPLE:

September 2024: Addition of another variable to assess the impact of previous infection on the risk of hospitalisation
...
- SOFTWARE EXPERIENCE (e.g., BASIC KNOWLEDGE OF R)
...
-   As of `r format(surveillance_date, format = "%B %d %Y")`, Fulton County has recorded `r nrow(linelist)` confirmed cases of COVID-19.\
-   As of `r format(surveillance_date, format = "%B %d %Y")`, Fulton County has recorded `r fmt_count(linelist, died == "Yes")` deaths.\
-   Among all confirmed cases of COVID-19 in Fulton County, `r fmt_count(linelist, hospitalized == "Yes")` required hospitalization.
...
As seen in Table \@ref(tab:regrtab), mortality was associated with being male `r inline_text(regress_tab, variable = gender, level = "Male", pattern = "(OR {estimate}; 95% CI {conf.low}, {conf.high})")`, being white `r inline_text(regress_tab, variable = eth_race, level = "White, NH", pattern = "(OR {estimate}; 95% CI {conf.low}, {conf.high})")`and longer hospital stays `r inline_text(regress_tab, variable = days_hosp, pattern = "(OR {estimate}; 95% CI {conf.low}, {conf.high})")`.
