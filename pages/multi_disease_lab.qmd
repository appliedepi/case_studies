...
already very clean, so we only need to standardize one column. We will also want to process the laboratory data frame to be one row per notification, so that it can be neatly linked to the notification data frame. 
...
Do this as you did with the notification data, using
...
As you saw in section 3.2, your dengue test provides results for
...
prioritizing positive results. You need to specify `desc` within
...
define the desired order of its levels. Don't forget to ungroup again at the end.
...
**Note**: You are lucky to work with such a straight-forward example of linkage! Usually you would need to really clean and check the ID column, or link on other columns like name and date fo birth. In Feveria, clinic staff are _fantastic_ at consistently allocating notification IDs to each patient, including on the sample forms sent to the lab, and then the lab staff are equally _brilliant_ at recording the notification ID in their lab systems so that the results can be linked back to the case.
...
notification_id, so one row in the notification data
...
To check if there were any lab result that were not linked to the notification data, you can use `anti_join()`. This time the `data_lab_cases`
...
Finally, to check the number of notifications without a result, you can either
...
in `data_linked` (as the `value` column comes from the lab data). 
...
This means that all cases in the notification data
...
cat("What proportion of cases in the notification data DID have a positive or negative result?", longmcq(opts))
...
cat("Which disease had the highest proportion of cases that remained suspected after linkage?", longmcq(opts))
...
#### **Task B: Assess the proportion of suspected cases that are indeed true cases**
...
cat("What proportion of suspected cases reported in 2024 became confirmed?", longmcq(opts))
...
cat("What proportion of suspected malaria cases became confirmed?", longmcq(opts))
...
cat("What proportion of suspected dengue cases became confirmed?", longmcq(opts))
```
:::
::::
...
Divide the number of confirmed cases (i.e. those with a positive result) by the number of confirmed plus discarded cases (i.e. those with either a positive or negative result). This gives a positivity rate, which approximates the proportion of suspected cases that were truly cases. Indeterminate results are excluded because they don't provide a clear outcome and would skew the positivity rate.
:::

::: {.callout-caution icon="false" collapse="true"}
## Click to see the solution (try it yourself first!)

Filter out suspected cases and then cross-tabulate, to see the proportion of originally suspected cases that become confirmed or discarded, among those with valid test results. 
...
opts <- c(
  answer = "Reporting confirmed cases can be more reliable and accurate when the proportion testing positive is low and lab testing is routine, thereby helping prevent over-estimation of disease burden",
...
Your surveillance unit wants to focus on confirmed cases in reporting. This is because lab testing is routine in Feveria, and so reporting suspected cases would be unnecessarily inaccurate, with a high proportion of suspected cases getting discarded.

The decision to publish suspected cases may be different in
other contexts. For example, if the positivity rate is high (most cases are true cases if tested), and testing itself is not common, or testing takes a long time and would result in delayed reporting, this would suggest that suspected case trends are sufficiently accurate and also more timely than waiting for laboratory confirmation. 
...
1. You used key R functions to clean, reshape, and link data frames, plus created new columns using logical conditions.
2. To inform the data processing, you conducted data inspections and checks along the way
3. You conducted a thorough descriptive analysis to understand the testing and notification data, before and after linkage. In response to your supervisor's original four questions, you can say:
...
4.  Finally, you have reflected on how the processes involved in notifiable disease surveillance systems and lab testing, for instance the transfer of data between clinics to labs, can affect data quality and completeness, and therefore your results. 
...
# Distribution of test results test
tabyl(data_lab_tests, test, value) |> 
    adorn_totals(where = "col") |> 
    adorn_percentages() |> 
    adorn_pct_formatting() |> 
...
::: {.callout-note appearance="minimal" icon="false"}
**Original authors**: Paula Blomquist and Alanah Jansen 

**Data source**: Fictional data provided by Applied Epi, with technical support provided by the CDC Global Surveillance, Laboratory, and Data Systems Branch in collaboration with TEPHINET. 
...
**Disclaimer**: The information presented in this exercise and the associated data files have been developed to help learners achieve the intended learning objectives. The contents are solely the responsibility of the authors and do not necessarily represent the official views of the Centers for Disease Control and Prevention, the Department of Health and Human Services, The Task for Global Health, Inc. or TEPHINET.
...
**Funding**: This case study was 100% supported by Grant or Cooperative Agreement number NU2HGH000044 funded by the Centers for Disease Control and Prevention (CDC). 