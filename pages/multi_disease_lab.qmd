...
# Linking, reshaping, and analysing notifiable disease surveillance data and laboratory data in R {#multi_disease_lab}

::: {.callout-note appearance="minimal" icon="false"}
**Tool**: R \| **Technical complexity**: Intermediate \| **Methodological complexity**: Basic \|
**Source:** Applied Epi and CDC DGHP GDLDSB (simulated data) \|
...
You are an epidemiologist working in the country of Feveria, in the national surveillance office. Your team already routinely collects and processes notifiable disease surveillance data as well as the lab results for those notifications. Your boss would like to transition the analysis  process into R, so that on a weekly basis you can monitor the disease trends and the performance of the surveillance systems. 

You have access to two routine datasets:
-    Notifiable disease data submitted to your surveillance unit routinely by clinics across the country via DHIS2
-    Laboratory data submitted by three large national laboratories, also submitted to your unit via DHIS2
...
1.  Use key R functions to clean string data, reshape datasets, link data sources, and create new variables using logical conditions to prepare data for analysis.
2.  Conduct data quality checks at multiple stages of the project and understand their importance for reliable analysis.
3.  Perform basic descriptive analyses to compare disease trends across different data sources, before and after linkage.
4.  Interpret differences in results across data sources and understand how these reflect the structure and design of the overall surveillance system.
...
Create a folder (e.g. 'multi_disease_lab' on your Desktop) for your work. To create an Rstudio project in your new folder, click `New Project…` in
...
<span style="color:blue">**Need to update packages below**</span>
...
Your office provides you with two files for your analysis, both updated as of 31st August 2025:

-   A notification-level data (*"notification_data.xlsx"*) with case information from 10 hospitals and clinics
-   Test-level data (*"lab_data.csv"*) submitted by three national laboratories for the 10 hospitals and clinics
...
Great! Thanks country office and Applied Epi! Now it's time to import the data
from that folder into RStudio, so you can analyse it.

**Task:** Import the two downloaded data files into your
...
# Notification data
notification_data_raw <- import("data/notification_data.xlsx")

# Lab data
lab_data_raw <- import("data/lab_data.csv")
...
-   If the contents of columns are clean and standardized in the notification_data linelist (e.g. sex, residential region, residential city, disease notified). Do you need to recode any of them?
...
printed to your RStudio console. Don't forget to use `tabyl(is.na())` to see how many missing values you have in each category.
...
from the `{epikit}` package. Then explore the function age_pyramid() from the {apyramid} package.You can find more about this function in the EpiRhandbook chapter [Demographic pyramids and
...
To further practise reproducible reports, \[link to
RMarkdown\].
