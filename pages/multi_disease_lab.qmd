...
Rstudio; R packages, functions and arguments, using pipes) \| **Source:** Applied Epi and Centers for Disease Control and Prevention (CDC) - Division of Global Health Protection (DGHP) Global Surveillance Laboratory and Data Systems Branch (GSLDSB) (simulated data) \| **Funded by:** TEPHINET
:::
...
-   **Feveria Central**: a large over-populated downtown area and oldest district in the Metropolis, with less reliable
...
opts <- c(
  "Onset date",
  "Date seen at Health Facility/Community",
  "Date of outcome",
  answer = "Date of test",
  "Date of birth"
)
...
If you inspect the data with `View(data_notif_raw)` and then click on the arrow at the top of each column to sort, you'll see that
...
could be inspected with this column.
...
more targets, and that it is only the gengue test which targets more
...
create a clean linelist that only contains information on the unique identifier, location of the case, disease, and the date of report.
...
and comparing the values in the original dataset and the clean one. Make sure that you used 'O' and not '0'!
...
#### **Task B: Count the number of cases tested in the 2024 data**
...
the rows from this dataset, and will just bring in the test results
...
data. This is because discarded cases are not relevant, and suspected cases eventually get their results, of which many are
negative.
...
data_linked_confirmed |> 
  ...
  labs(fill = "District of residence",
       ...
       y = "Count",
       subtitle = "Number of confirmed cholera, dengue, malaria, typhoid fever, and yellow fever cases by week in Feveria Central, 2024") +
...
Now you can see an epicurve per disease, with the colouring
...
data_linked_confirmed |> 
  ...
  labs(fill = "District of residence",
       x = "Date reported by clinic",
       y = "Count",
       subtitle = "Number of confirmed cholera, dengue, malaria, typhoid fever, and yellow fever cases by week in Feveria Metropolis, 2024") +
  scale_fill_manual(values = c("navy", "lightblue", "seagreen")) +
  scale_x_date(date_breaks = "1 month", 
               date_labels = "%d %b") +
  theme(legend.position="bottom",
...
cat("When was the first dengue case reported in **Feveria Metropolis**?", longmcq(opts))

opts <- c(
  answer = "22rd August 2024",
  "18th November 2024",
  "25th December 2024"
)
...
Group the data by disease and then summarize by start and end date to look at the overall timeline in Feveria Metropolis.
...
Use `filter()` to then look at the region you're interested in.
...
Wow! You have made fantastic progress cleaning, linking, and preparing notifiable disease surveillance and laboratory data for analysis. The data is now much cleaner and more organized, and you have already begun to uncover meaningful insights.

Throughout this process, you mastered key R functions to clean, reshape, and link datasets, creating new columns using logical conditions. You performed thorough data quality checks to ensure reliable results, and you compared disease trends across sources before and after linkage, gaining a deeper understanding of how the surveillance system is structured.
...
**License**: This case study is under a [CC BY-NC-SA 4.0 license](https://creativecommons.org/licenses/by-nc-sa/4.0/legalcode.en). For more information about sharing and adapting this case study, see the [associated deed](https://creativecommons.org/licenses/by-nc-sa/4.0/).