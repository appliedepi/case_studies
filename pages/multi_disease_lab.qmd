...
You are an epidemiologist working in a regional surveillance office in Feveria Metropolis, the largest region of the tropical country Feveria. There are three districts within Feveria Metropolis:

- Feveria Central, the large over-populated downtown area. This is also the oldest distict within the Metropolis, with less reliable water and sanitation infrastructure. 
- Lake Minara, a lake area with good infrastructure but many mosquitoes in the warmer months of the year
- Kasara, the rural countryside area away on the other side of Feveria central

It is January 2025, and your boss would like you to transfer the routine processing of notifiable disease data into R. The processing currently uses Excel. 

She asks that you write code to import, clean, link, and analyse the following linelists:

- **2024 Notifiable Disease Surveillance Data:** This is the data your surveillance unit gets regularly from clinics throughout the Feveria Central region. It includes details about cases suspected of having Dengue, Malaria, Cholera, Typhoid Fever, and Yellow Fever, all diagnosed by clinics based on the patients' symptoms.
- **2024 Laboratory Data:** This data comes from lab test results, specifically from three major labs in Feveria Central. These results are for samples taken from those suspected notifiable disease cases mentioned above.
...
1.  Use key R functions to clean data, reshape datasets, link
    data sources, and create new variables using logical conditions to
...
-   Test-level data (*"lab_data.csv"*) submitted by three
    laboratories conducting testing for the 5 health centres
    
For this case study, you can access the data via AppliedEpi's very useful data repository,
...
An efficient function for initial data exploring is `skim()` from the `{skimr}` package, as it gives you a lot of information on data structure and content. The `names()` function will show you the different column names in your data. 

You can also use the `ncol()` and `nrow()` functions to simply count the numbers of columns and rows in the data. Do you know what to put inside the brackets? 
...
The function `skim()` from the `{skimr}` package will again be useful here. You can also use the `class()` function to look at the class if particular columns - do you remember how to specify the column of interest in this function? Again, the easiest approach is to look at the RStudio Environment. 
...
There are often more records in the lab data than in the clinical data. If you inspect the data with `View(data_notif_raw)`, you'll see that several rows share the same notification_id. This can happen when multiple targets are tested from the same sample (same sample ID), or when a case is retested (resulting in a different sample ID).
...
Use `class()` with the one date column in the data - you will see that R does recognise it as a date. This means R understands the order of values in this column (e.g. from oldest to newest) and trends over time could be inspected with this colmumn. 
```{r}
...
Due to quality and data storage issues, your team recommends that you create a clean linelist that only contains information on the disease, date of report, location of the case, and the unique identifier. 
...
You already know from your data inspection that the values for district are not standardised. Add two `mutate()` functions to replace the `residential_district` column, to:
...
### Step 4.2: Perform simple descriptive analysis 

Your colleagues inform you that each row, identified by the notification_id, represents an individual case. A person may appear several times in the data however, for instance if they were diagnosed with suspected malaria one month and then suspected cholera another month. Do not worry about such cases. 

Use the `tabyl()` function for the following tasks. 
...
Using `tabyl()`, we can see that there were 533 suspected cases of Malaria in Feveria Centra in 2024, and only 25 suspected cases of Typhoid. 
```{r eval=T}
...
Use `tabyl()` to cross-tabulate the disease and district of residence columns. You can also add various `adorn` functions from the `{janitor}` package to build on your table to see percentage distributions, e.g. `adorn_percentages()`, `adorn_pct_formatting()`, and `adorn_ns()`
...
## Step 5. Deduplicate and describe the laboratory data

From your earlier work in step 3, you have found that the laboratory data contains only testing data, and no patient information, with samples identified with a notification ID and Sample ID. This data is long, with several rows for Dengue tests, so will need some reshaping before being analysed. 

The columns and contents are already clean, 

### Step 5.1 Deduplicate

This will be done with either distinct() or slice() - need ot check

Need to create an object with one row per test
Then one row per notification

### Step 5.2 Describe

Look at numbers of tests and notifications by disease
Count by lab also

## Step 6. Linkage

## Step 7. Descriptive analysis of linked data
