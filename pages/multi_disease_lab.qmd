...
You are an epidemiologist working in the country of Feveria, in the
national surveillance office. Your team already routinely collects and
processes notifiable disease surveillance data as well as the lab
results for those notifications. Your boss would like to transition the
analysis process into R, so that on a weekly basis you can monitor the
disease trends and the performance of the surveillance systems.

You have access to two routine datasets: 
- Notifiable disease data
submitted to your surveillance unit routinely by clinics across the
country via DHIS2 
- Laboratory data submitted by three large national
laboratories, also submitted to your unit via DHIS2
...
1.  Use key R functions to clean string data, reshape datasets, link
...
Your office provides you with two files for your analysis, both updated
as of 31st August 2025:

-   A notification-level data (*"notification_data.xlsx"*) with case
    information from 10 hospitals and clinics
-   Test-level data (*"lab_data.csv"*) submitted by three national
    laboratories for the 10 hospitals and clinics
...
**Task:** Import the two downloaded data files into your R environment.
...
-   If the contents of columns are clean and standardized in the
    notification_data linelist (e.g. sex, residential region,
    residential city, disease notified). Do you need to recode any of
    them?
...
printed to your RStudio console. Don't forget to use `tabyl(is.na())` to
see how many missing values you have in each category.
