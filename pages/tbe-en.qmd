...
cat("QUESTION: Based on the results of this model, is there a significant association between age and hospd?", longmcq(opts))
...
#### Step 3.3: Check if the association between age and length of hospitalisation varies by sex
...
-   **What do you observe?**
The lines for female and male patients have different slopes, indicating that the association between age and hospitalization days is modified by sex.
...
We can check whether TBE cases with high blood pressure have a significant longer stay in hospital compared with those without high blood pressure either by carrying out a simple statistical test or through univariate regression. We will do both
...
In this case we have independent data, two groups (hypertension yes/no) and a quantitative dependent (outcome) variable (length of hospitalisation), so we will choose either a **t-test** or **Wilcoxon rank-sum test** (also known as Mann-Whitney). We'd do a t-test if length of hospitalisation is normally distributed and Wilcoxon if it isn't.

**Task** Check visually whether length of hospitalisation is normally distributed. 
...
However, if we want to to add a normal curve, we'd need to plot the density and not the frequency, as you may rembember from the histogram we built above in Step 2.3. 
</br>
...
#### Step 4.2: Univariate linear regression


Carry out a univariate linear regression to check whether hypertension is associated with length of hospitalisation.
By now you should be able to do this on your own, but if you have any doubts look at Step 3.2 or to the [Regression Chapter](https://epirhandbook.com/new_pages/regression.html) of the EpiRhandbook.
...
tidy(hyper_hospd_lm)                        #remember that we can also get them with the function summary()
```
...
The intercept (41.3) is the predicted mean of the hospitalization days when “hyper” is zero. In this example this refers to the mean hospitalization days for those without hypertension. 

The coefficient for “hyper” indicates the average amount of change in hospitalization days that would be predicted by a one unit increase in “hyper”. Here it means that those with hypertension have 3.9 hospitalization days more than those without hypertension. This effect is statistically significant (see the p value).


### **Step 5: Check if there is a linear association between length of hospitalisation and the rest of variables**

Carry out a linear model for each variable that we have not explored yet to see if there is a linear association between length of hospitalisation and them.
...
We have seen already how to write a linear formula with lm(). You can then use either summary() from {base} or tidy() from {broom} to look at the coefficient and significance value. You could write these functions for each variable, but you could also try to use map() from {purrr} to do them all at once, as we did in the Step 2.4. Have a look at the Chapter on  [Purrr](https://epirhandbook.com/new_pages/iteration.html#iter_purrr) in the EpiRhandbook if you have any doubts.
...
Statistically significant ($p<0.05$) univariable effects were observed for sex, other comorbidities, vaccination and monophasic disease course but not for large tick at removal. Women had on average 12.4 days shorter hospitalization stay than men, other comorbidities increase the hospitalization by 4.1 days, vaccination reduced it by 8.4 days, and monophasic disease course increased it by 3.4 days.


### **Step 6: Adjusted analysis**
...
tidy(hyper_hospd_adj_lm)                                   # finally we do tidy on each model
```
...
Adjusting for age, we find that hypertension is associated with a 3.8 increase in the days of hospitalizations. This is only a slight change compared to the effect in the unadjusted model (where the coefficient was 3.9).
...
We did not allow the association between age and hospitalization to vary by levels of hypertension. So we force them to be parallel by the way we specify the model. If we want the slopes to differ, we would need to allow for an interaction term. We’ll get to that later.
...
We saw in Step 3.3 that the effect of age on length of hospitalisation is modified by sex. We knew this because the slopes were different for males and females. Given this information add now to the model an interaction term between sex and age.
...
Review Step 4.1 and have a look at the EpiRhandbook Chapter on [Simple statistical tests](https://epirhandbook.com/new_pages/stat_tests.html) if you still have doubts.
