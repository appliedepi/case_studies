---
title: "Fulton Covid-19 Case Study Script"
author: "AppliedEpi Team"
date: " 30 June 2021"
output: 
...
pop <- import(
     here("data", "covid_example_data", "fulton_population.xlsx")
)
```
...
names - and rename() from {dplyr} for manually renaming.
2
For more details see: 
...
Here we are going to make sure that date columns are considered dates by R, that
age is a numeric column and that the zip code is a factor column. We are also 
going to remove values that are inconsistent (e.g. negative age). 

We'll carry out these changes using across() from {dplyr}, which allows you to apply a
function across multiple specified columns. See
https://epirhandbook.com/cleaning-data-and-core-functions.html#clean_across

Note: The {base} way to convert columns to Date class is with as.Date(). If the
...
# To convert every date column into a date we can use the as.Date() function (or lubridate functions such as dmy() within mutate() for each date column, but we can also apply this function automatically to all date columns using the code below
...
Here we create a new column called "epiweek" rounding the date of report to the nearest Wednesday 
For more information:
https://epirhandbook.com/en/grouping-data.html?q=floor_Date#linelist-cases-into-weeks
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
...
Here we show how to create new numeric variables for the number of days between 
two dates. 

For more details see: 
https://epirhandbook.com/en/working-with-dates.html?q=time%20difference#date-intervals
...
Here we are going to clean all character and categorical columns recoding missing to an "Unknown" category and re-ordering the levels of some of the factor columns 
...
Here we create a new column merging information from the ethnicity column and the race column
 ...
 linelist <- linelist %>%  
     ...
     mutate(eth_race = case_when(
       ...
       # case_when() evaluates in order (if, else if, else if, ... else)
          case_eth  == "HISPANIC/LATINO"                           ~ "Hispanic, all races", 
          case_race == "ASIAN"                                     ~ "Asian, NH", 
          case_race == "BLACK"                                     ~ "Black, NH",
          case_race == "WHITE"                                     ~ "White, NH",
      # find all instances of NATIVE (covers AMERICAN INDIAN/ALASKA NATIVE **AND** NATIVE HAWAIIAN/PACIFIC ISLANDER)
          str_detect(case_race, "NATIVE")                          ~ "Other, NH",
          case_race == "OTHER"                                     ~ "Other, NH", 
          TRUE                                                ~ "Unknown"
     )) %>% 
...
This document will be analysing COVID-19 data in Fulton County by time, place, and person to produce descriptive 
tables and graphs. 
...
Here we use the {janitor} tabyl() function to get case counts by reporting week 
(as well as the percentage these contribute to the overall). 

We then create use {flextable} to produce a clean output table. We use this table to assess where the peak in number of cases was reached
...
```{r epicurve, fig.cap = "COVID-19 cases by reporting week and hospitalisation status in Fulton County", fig.id = "epicurve"}
...
As it can be observed from above in Figure \@ref(fig:epicurve) there are clear trends in reporting patterns: cases increase in waves and peak in the week starting on the `r filter(epiweek_table, epiweek != "Total") %>% summarise(which.max(n)) %>% pull() %>% epiweek_table[., "epiweek"] %>% as.Date() %>% format(., format = "%d %B %Y")`. 


\pagebreak 

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The tag "BLOCKK_LANDSPACE_START" below will change the page to landscape in the word document output. 
...
Table \@ref(tab:demogtab) shows the demographic characteristics of the cumulative and recent COVID-19 cases and COVID-19 deaths in Fulton County. 
...
dem_gender <- linelist %>% 
  tabyl(case_gender) %>% 
  select(Characteristic = case_gender, n, percent)
...
demographic_vars <- c("case_gender", "age_group", "eth_race")
...
Above, in Table \@ref(tab:demogtab) it could be observed that, even though a larger proportion of cases occur in females, more deaths occur in males. The age pyramid below in \@ref(fig:agepyramid) shows that the larger proportion of cases observed in females is consistent across most age groups, with the exception of those aged 0-9 and those aged 60-69, where a similar proportion of females and males can be observed. 
...
linelist_2g <- linelist %>% 
  ...
  mutate(across(.cols = c(case_gender, age_group), 
                .fns = ~{
...
Table \@ref(tab:demogtab) also showed that, although most cases occur in under 40 years old,  the vast majority of deaths occur in people aged 70+. This is confirmed by Figure \@ref(fig:agehospplot) below, which suggests an 
association between older age, length of hospital stay and mortality. We see similar individual associations in Table \@ref(tab:regrtab), though these do not take into consideration interaction or confounding, so they should be interpreted with caution. 
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// age_hosp_died \\\
--------------------------------------------------------------------------------
...
Finally, in \@ref(tab:demogtab), looking at the distribution of cases according to race, we observe that the percentage of deaths in the Black population is larger than one would expect. In fact, \@ref(fig:ethnicityplot) below confirms that in Fulton County the black population is disproportionally affected by COVID-19.
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// ethnicity_counts \\\
--------------------------------------------------------------------------------
...
The \@ref(tab:ziptab) below can be used to identify areas where the incidence of SARS-CoV-2 infection is rapidly increasing in Fulton County, so that public health authorities can consider the implementation of measures aimed at controlling the epidemic.
...
zip_counts %>% 
  ...
  select(case_zip, recent, recent_inc, previous, previous_inc, perc_change) %>% 
  # initiate {flextable} to produce styled output table
  flextable() %>% 
...
descriptive_vars <- c("case_gender", 
                      "age_group",
                      "eth_race",
                      symptom_vars,
                      "hospitalized",
...
rf_data <- linelist %>% 
  ...
  select(died_covid, case_age, all_of(descriptive_vars)) %>% 
  ...
  mutate(gender = fct_relevel(case_gender, "Female", "Male")) %>% 
  ...
  drop_na(any_of(c("died_covid", "case_age", descriptive_vars)))
...
rf_data %>%
  ...
  select(died_covid, gender, eth_race, case_age, days_hosp) %>%
