...
2. Create an Rstudio project in the folder tbe_en. If you are unsure on how to do that, read the EpiRhandbook Chapter on [R projects](https://epirhandbook.com/en/new_pages/r_projects.html)

3. Inside the folder "tbe_en": Subfolder "data" contains a raw data file named *tbe.RDS*. This is the only data file you will use in this case study. 

4.  Subfolder scripts should be used to save any scripts related to the
    analysis. Inside "backup" you will find a solution script with the code of the case study named *tbe_lr_backup.R*. You will also find an image which corresponds to the proposed DAG solution.
...
The intercept (41.3) is the predicted mean of the hospitalisation days when “hyper” is zero. In this example this refers to the mean hospitalisation days for those without hypertension. 

The coefficient for “hyper” indicates the average amount of change in hospitalisation days that would be predicted by a one unit increase in “hyper”. Here it means that those with hypertension have 3.9 hospitalisation days more than those without hypertension. This effect is statistically significant (see the p value).
